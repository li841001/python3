...
def log_in():
    ...
    #   写入cookies
    cookies_dict = requests.utils.cookiejar_from_dict(login.cookies)
...
def resturant():
    ...
    latitude = str(location_detail['latitude'])
    longitude = str(location_detail['longitude'])
    params = {'extras[]': 'activities',
            'geohash': 'wtmkrd5xd21j',
            'latitude': latitude,
            'limit': '24',
            'longitude': longitude,
            'offset': '0',
            'terminal': 'web'}
    res_json = requests.get(url_rst,headers=headers,params=params).json()
    resturant_list = []
    for resturant in res_json:
        resturant_info = ['名称：'+resturant['name'],'评分：'+resturant['rating'],'简介：'+resturant['promotion_info'],'店铺链接：'+resturant['scheme']]
